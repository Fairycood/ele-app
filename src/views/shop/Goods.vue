<template>
  <div class="goods">
    <!-- 商家推荐 -->
    <div class="recommend" v-for="(rec, index) in data.recommend" :key="index">
      <p class="recommend-name">{{ rec.name }}</p>
      <div class="recommend-wrap">
        <ul>
          <li v-for="(item, index) in rec.items" :key="index">
...
//导入子组件
import CartControll from "./childComp/CartControll.vue";
import ShopCart from "./childComp/shopCart.vue";
import Food from "./childComp/Food.vue";
...
export default {
  ...
  data() {
    return {
      data: null, //存储商品信息数据
      menuScroll: null, //左侧滚定对象
      foodScroll: null, //右侧滚动对象
      scrollY: 0, //右侧滚定对象所滚动的y轴的距离
      listHeight: [], // 12个区列表高度
      selectFood: null, //存储用户要查看的商品的详情信息
