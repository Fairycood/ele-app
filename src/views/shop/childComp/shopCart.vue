          ...
          <div class="entityList-cartbodyScroller">
            <ul class="entityList-cartlist">
              ...
              >
                ...
                <cart-controll :foods="item"></cart-controll>
              </li>
            </ul>
          </div>
        </div>
    ...
    >
      ...
      <div class="bottomNav-cartInfo" @click="showCartView = !showCartView">
        ...
        <p class="bottomNav-cartdelivery">
          另需配送费{{ data.rst.float_delivery_fee }}元
        </p>
      </div>
...
export default {
  ...
  data() {
    return {
      totalCount: 0, //用户所选商品的总个数
      totalPrice: 0, //用户所选商品的总价格
      selectFoods: [], //存储用户所选的商品信息
      showCartView: false, //是否显示购物车的弹窗
    };
  },
  computed: {
    //判断用户所选商品是否为空，计算用户所选商品的总个数以及总价格，并把用户所选的商品存储起来
    isEmpty() {
      let empty = true;
      this.totalCount = 0;
      this.totalPrice = 0;
      ...
      this.data.recommend.forEach((recommend) => {
